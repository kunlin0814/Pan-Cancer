...
meta_data_file <- paste(lab_path, "pancancer\\metadata\\merged\\metadata_summary_01_13_2021.txt", sep="");
base_dir <- paste(lab_path, "pancancer\\QC\\", sep="");
shared_variants_fill_list <- list("MT CUK"=paste(base_dir, "discovery\\mammary_normal_tumor_match.proportions.txt", sep=""),
					"GLM JL"=paste(base_dir, "discovery\\glioma_normal_tumor_match.proportions.txt", sep=""),
					"OM SC1"=paste(base_dir, "discovery\\melanoma_normal_tumor_match.proportions.txt", sep=""),
					"Broad PRJNA247493"=paste(base_dir, "discovery\\PRJ93_normal_tumor_match.proportions.txt", sep=""),
					"HSA BI"=paste(base_dir, "discovery\\hemangiosarcoma_normal_tumor_match.proportions.txt", sep=""),
					"MT SNU"=paste(base_dir, "validation\\PRJNA552905_MC_WES.proportions.txt", sep=""),
#					"MT UGA"=paste(base_dir, "validation\\lab_MT_WES.proportions.txt", sep=""),
#					"OM SC2"=paste(base_dir, "validation\\PRJEB7540_OM.proportions.txt", sep=""),
					"OSA TGen"=paste(base_dir, "validation\\Tgen_TrueN_Tpairs_WES.proportions.txt", sep=""),
#					"OSA SC"=paste(base_dir, "validation\\PRJEB7540_OSA.proportions.txt", sep=""),
					"HSA UPenn"=paste(base_dir, "validation\\PRJNA417727_HSA_WES.proportions.txt", sep=""));

failed_sequencing_file <- paste(base_dir, "excluded_samples_merged_12_22_2020.txt", sep="");

################# Output files ######################## 
# make sure to modify the paths to the correct ones
output_pdf <- paste(base_dir, "tumor_normal_pair_QC_results.pdf", sep="");
output_csv <- paste(base_dir, "tumor_normal_pairing_jitterplot_data.csv", sep="");
output_QC_results <- paste(base_dir, "tumor_normal_pair_QC_results.txt", sep="");
...
for(bp_dataset in dataset_order) {
	...
	bp_annt <- c(bp_annt, paste(self_count, "/", length(sample_indices), sep=""), "");
}
...
pdf(output_pdf, height=5, width=6.84);
	...
	p <- p + labs(y="Fraction of shared variants", title="Tumor/normal pairing accuracy", fill="");
	...
	bar_plot <- ggplot(barplot_data, aes(x=barplot_data$x, y=barplot_data$y, fill=barplot_data$fill)) + 
		...
		coord_flip() + labs(title="", x="", y="Best match", fill="") +
