...
for (index in 1:length(tumor_type)) {
  ...
  for (i in 1:length(total_variants_each_tumor)) {
    ...
    for (other_variant in  total_variants_each_tumor) {
      if (other_variant != each_variant) {
        nubmersamples_others_variants <-
          length(unique(each_tumor_type[chrom_loc == other_variant, .(sample_names)][['sample_names']]))
        numersamples_others_withoutvariants <-
          length(unique(each_tumor_type$sample_names)) - others_variants
        total_others <- total_others + nubmersamples_others_variants
        total_others_without <-
          total_others_without + numersamples_others_withoutvariants
      }
  ...
  for (i in 1:length(total_ensembl_each_tumor)) {
    ...
    for (other_ensembl in  total_ensembl_each_tumor) {
      if (other_ensembl != each_ensmbl) {
        nubmersamples_others_variants <-
          length(unique(each_tumor_type[ensembl_id == other_ensembl, .(sample_names)][['sample_names']]))
        numersamples_others_withoutvariants <-
          length(unique(each_tumor_type$sample_names)) - others_variants
